...
const envFile =
  envFileArgIndex >= 0
    ? process.argv.at(envFileArgIndex + 1)
...
function checkRequiredGroup(name, keys, env) {
  ...
  if (missing.length === 0) {
    return {
      name,
      status: "pass",
      detail: "all required values are configured",
    };
  }
...
function checkSimulationMode(env) {
  ...
  return {
    ...
    status: "fail",
    detail:
      "DEMO_BANKING_MODE must be true and BANKING_RAILS_MODE must be simulation",
  };
}
...
async function main() {
  ...
  if (shouldCheckExternal && isConfigured(env.DATABASE_URL)) {
