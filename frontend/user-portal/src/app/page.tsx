...
type MoneyTransferRealtimeEvent = {
  reference: string;
  amount: number | string;
  status: string;
};

type TransferTargetType = "beneficiary" | "own" | "other";
...
export default function Home() {
  ...
  const [customerAccounts, setCustomerAccounts] = React.useState<
    CustomerAccount[]
  >([]);
  const [customerBeneficiaries, setCustomerBeneficiaries] = React.useState<
    CustomerBeneficiary[]
  >([]);
  ...
  const [wizardStep, setWizardStep] = React.useState<
    "beneficiary" | "review" | "2fa" | "otp" | "receipt"
  >("beneficiary");
  const [sourceAccountId, setSourceAccountId] = React.useState("");
  const [targetType, setTargetType] =
    React.useState<TransferTargetType>("beneficiary");
  ...
  const [activeTransferSandboxCode, setActiveTransferSandboxCode] =
    React.useState("");
  const [activeTransferNeeds2fa, setActiveTransferNeeds2fa] =
  ...
  const handleEmailVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEmailError(
        getErrorMessage(error, "Verification code is invalid or expired."),
      );
    } finally {
  ...
  const handleQuickDeposit = async (e: React.FormEvent) => {
    ...
    try {
      if (customerAccounts.length > 0) {
        ...
        setDepositSuccess(
          `Successfully deposited INR ${Number(depositAmount).toLocaleString()}!`,
        );
        setDepositAmount("");
      ...
      console.error("Failed deposit simulation:", error);
    } finally {
  ...
  const handleTransferSubmitDetails = (e: React.FormEvent) => {
    ...
    const selectedSource = customerAccounts.find(
    ...
    try {
      const result = (await apiFetch("/transfers/initiate", {
        ...
        body: JSON.stringify({
          ...
          type: transferType,
          scheduledFor:
            transferType === "SCHEDULED" ? scheduledDate : undefined,
        }),
      })) as TransferInitiationResponse;
      ...
      setTransferError(
        getErrorMessage(error, "Failed to initialize secure transfer rails."),
      );
    } finally {
  ...
  const handleVerify2Fa = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTransferError(
        getErrorMessage(error, "Invalid 2FA authentication token."),
      );
    } finally {
      setTransferLoading(false);
    }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const result = await apiFetch(
        `/transfers/${activeTransferId}/verify-otp`,
        {
          method: "POST",
      ...
      setTransferError(getErrorMessage(error, "Invalid or expired OTP code."));
    } finally {
      setTransferLoading(false);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,oklch(0.92_0.05_184_/_0.35),transparent_32rem)] px-4 py-12">
        <Card className="w-full max-w-lg border-border/60 bg-card/65 backdrop-blur-md shadow-2xl p-4">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              To protect your identity and follow banking guidelines, please
          ...
          <CardContent className="space-y-6">
            <div className="rounded-lg bg-muted/50 p-4 text-sm space-y-2 text-muted-foreground">
              ...
              <ul className="list-disc pl-5 space-y-1">
                <li>
                  Government ID Proof copy (Aadhaar, PAN, Passport, or License)
                </li>
    ...
    const getStageBadge = (status?: string) => {
      ...
      if (displayStatus === "APPROVED") {
        return (
          <Badge
            variant="outline"
            className="border-emerald-500 bg-emerald-500/10 text-emerald-500 font-semibold"
          >
            {displayStatus}
          </Badge>
        );
      ...
      <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,oklch(0.92_0.05_184_/_0.35),transparent_32rem)] px-4 py-12">
        <Card className="w-full max-w-xl border-border/60 bg-card/65 backdrop-blur-md shadow-2xl">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Our operations and compliance teams are currently reviewing your
              documents.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
      ...
      <header className="sticky top-0 z-10 border-b bg-background/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6">
          ...
          <div className="ml-auto hidden min-w-64 items-center gap-2 md:flex">
            ...
            <Input
              aria-label="Search banking workspace"
          ...
          <div className="max-w-3xl space-y-3">
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline">
                Customer ID: {user.id.slice(0, 8).toUpperCase()}
              </Badge>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Add mock funds to your savings account in the sandbox.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {depositSuccess && (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Multi-step transfer checkout flow with dynamic checks.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {transferError && (
                ...
                {wizardStep === "beneficiary" && (
                  <form
                    onSubmit={handleTransferSubmitDetails}
                    className="space-y-4"
                  >
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          onValueChange={(value) =>
                            setTargetType(value as TransferTargetType)
                          }
                          value={targetType}
                        >
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <div className="space-y-1">
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="IMPS_SIMULATION">
                              IMPS (Immediate Transfer)
                            </SelectItem>
                            <SelectItem value="NEFT_SIMULATION">
                              NEFT Simulation
                            </SelectItem>
                            <SelectItem value="RTGS_SIMULATION">
                              RTGS Simulation
                            </SelectItem>
                            <SelectItem value="UPI_SIMULATION">
                              UPI (Instant VPA)
                            </SelectItem>
                ...
                {wizardStep === "review" && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs border rounded-lg p-3 bg-muted/20">
                      ...
                      <div>
                        ...
                        <strong className="text-primary font-bold">
                          INR {Number(transferAmount).toLocaleString()}
                        </strong>
                      </div>
                      {transferType === "SCHEDULED" && (
                    ...
                    <div className="flex gap-3">
                      ...
                      <Button
                        onClick={() => setWizardStep("beneficiary")}
                        variant="outline"
                        className="flex-1"
                      >
                        Modify Parameters
                  ...
                  <form onSubmit={handleVerify2Fa} className="space-y-4">
                    <div className="text-center space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Please open your Google Authenticator app and input your
                        secure code.
                      </p>
                    </div>
                ...
                {wizardStep === "otp" && (
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    <div className="text-center space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        We've sent a 6-digit transaction authorization key to
                        your email.
                      </p>
                    </div>
                  ...
                  <div className="space-y-4 animate-fade-in">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs p-3 border rounded-lg bg-card/65">
                      ...
                      <div>
                        ...
                        <Badge
                          variant="outline"
                          className="border-emerald-500 bg-emerald-500/10 text-emerald-500 font-semibold uppercase text-[9px] py-0 px-1.5"
                        >
                          {transferType === "SCHEDULED"
                            ? "SCHEDULED"
                            : "COMPLETED"}
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Live notifications over WebSocket channels.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="rounded-xl border border-border/40 bg-muted/20 p-4 min-h-[14rem] max-h-[18rem] overflow-y-auto space-y-2.5 font-mono text-[11px] leading-relaxed">
                  {notifications.length === 0 ? (
                    <span className="text-muted-foreground block text-center pt-12">
                    ...
                    notifications.map((note, index) => (
                      <div
                        key={index}
                        className="border-b border-border/20 pb-1.5 last:border-0 text-foreground"
                      >
                        {note}
                      </div>
                    ))
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Block or freeze digital debit cards instantly
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-2">
                <Button variant="outline" className="justify-start text-xs">
                  <CreditCard className="size-4 mr-2" /> Temporarily Freeze
                  Debit Card
                </Button>
                <Button
                  variant="outline"
                  className="justify-start text-xs text-destructive hover:bg-destructive/5 hover:text-destructive"
                >
                  <ShieldAlert className="size-4 mr-2" /> Report Lost /
