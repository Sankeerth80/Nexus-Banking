...
export default function AdminDashboard() {
  ...
  const [selectedRequest, setSelectedRequest] =
    React.useState<KycDetails | null>(null);
  ...
  const fetchQueue = React.useCallback(async () => {
    ...
    try {
      ...
      setError(
        getErrorMessage(error, "Failed to retrieve pending KYC requests."),
      );
    } finally {
  ...
  const handleSelectRequest = async (customerId: string) => {
    ...
    try {
      ...
      setError(getErrorMessage(error, "Failed to load request details."));
    } finally {
  ...
  const handleReviewSubmit = async (status: "APPROVED" | "REJECTED") => {
    ...
    try {
      ...
      setSuccess(
        `KYC stage "${step}" has been successfully ${status.toLowerCase()}!`,
      );
      setReviewComment("");
      ...
      setError(getErrorMessage(error, "Failed to submit review."));
    } finally {
  ...
  const getActiveStepInfo = () => {
    ...
    const isAuthorized =
      ...
      (step === "DOCUMENT" && user.role === "BRANCH_MANAGER");
        ...
        <section className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl space-y-3">
            <div className="flex flex-wrap gap-2">
              <Badge className="bg-primary hover:bg-primary/95">
                Ops Workspace
              </Badge>
          ...
          <div className="flex gap-2">
            ...
            >
              <RefreshCw
                className={`size-3.5 ${queueLoading ? "animate-spin" : ""}`}
              />{" "}
              Refresh Queue
            </Button>
          </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[1.1fr_1.3fr]">
          ...
          <Card className="min-h-[20rem]">
            <CardHeader>
              ...
              <CardDescription>
                Select a customer request to inspect documents and submit stage
                feedback.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {queueLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-muted-foreground text-center">
                  ...
                  <p className="text-xs">
                    No pending applications require reviews currently.
                  </p>
                </div>
              ) : (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {pendingRequests.map((req) => {
                        const isSelected =
                          selectedRequest?.customerId === req.customerId;
            ...
            {detailsLoading ? (
              <div className="flex flex-col items-center justify-center py-24 space-y-4">
                ...
                <p className="text-xs text-muted-foreground animate-pulse">
                  Loading files and review history...
                </p>
              </div>
            ) : !selectedRequest ? (
              <div className="flex flex-col items-center justify-center py-24 text-muted-foreground text-center px-4">
                ...
                <p className="text-xs max-w-xs mt-1">
                  Choose a customer from the queue to review verification
                  details, preview credentials, and submit maker-checker
                  feedback.
                </p>
              </div>
            ) : (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedRequest.documentComment && (
                        <p className="text-[11px] text-muted-foreground italic pl-3 pb-1">
                          Comment: {selectedRequest.documentComment}
                        </p>
                      )}
                      ...
                      {selectedRequest.complianceComment && (
                        <p className="text-[11px] text-muted-foreground italic pl-3 pb-1">
                          Comment: {selectedRequest.complianceComment}
                        </p>
                      )}
                      ...
                      {selectedRequest.riskComment && (
                        <p className="text-[11px] text-muted-foreground italic pl-3 pb-1">
                          Comment: {selectedRequest.riskComment}
                        </p>
                      )}
                      ...
                      {selectedRequest.branchComment && (
                        <p className="text-[11px] text-muted-foreground italic pl-3">
                          Comment: {selectedRequest.branchComment}
                        </p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        ...
                        {selectedRequest.idDocUrl ? (
                          <a
                            href={selectedRequest.idDocUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex flex-col items-center justify-center h-20 rounded border bg-muted/40 hover:bg-muted/70 transition-all text-primary font-semibold text-xs text-center p-1.5"
                          >
                            <FileText className="size-5 mb-1" />
                            View ID Copy
                          </a>
                        ) : (
                          <div className="flex items-center justify-center h-20 rounded border bg-muted/20 text-muted-foreground text-[10px]">
                            No File
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {selectedRequest.photoUrl ? (
                          <a
                            href={selectedRequest.photoUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="block border rounded h-20 overflow-hidden bg-muted/30 hover:opacity-85 transition-all"
                          >
                            <img
                    ...
                    {activeStepInfo?.completed ? (
                      <div className="flex items-center gap-2 rounded-lg bg-emerald-500/10 text-emerald-500 p-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-xs">
                            This account has been activated and savings account
                            provisioned.
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="rounded-lg border bg-muted/30 p-3.5 space-y-2">
                          ...
                          <div className="flex items-center justify-between text-xs">
                            <span className="font-semibold text-foreground">
                              {activeStepInfo.stepLabel}
                            </span>
                            <Badge
                              variant="outline"
                              className="border-amber-500 bg-amber-500/10 text-amber-500 font-semibold animate-pulse"
                            >
                              Active Review
                            </Badge>
                          </div>
                          ...
                          {activeStepInfo.isAuthorized ? (
                            <div className="text-emerald-500 text-xs font-semibold flex items-center gap-1">
                              <BadgeCheck className="size-4" /> You are
                              authorized to review this step.
                            </div>
                          ) : (
                            <div className="text-destructive text-xs font-semibold flex items-center gap-1">
                              <ShieldAlert className="size-4" /> Your active
                              role is not authorized for this step.
                            </div>
                          )}
                        </div>
