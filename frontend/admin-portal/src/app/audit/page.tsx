      ...
      <header className="sticky top-0 z-10 border-b bg-background/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6">
          ...
          <nav className="flex items-center gap-4 ml-6">
            ...
            <Link href="/audit" className="text-sm font-semibold text-primary transition-colors">
              Auditing & Logs
            </Link>
          </nav>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:py-6">
        <section className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={handleRefresh} variant="outline" className="gap-1.5 text-xs">
        ...
        <Tabs defaultValue="transfers" className="space-y-4">
          <TabsList className="bg-muted/40 p-1 border rounded-lg max-w-xs">
            <TabsTrigger value="transfers" className="text-xs">Transaction Ledger</TabsTrigger>
            <TabsTrigger value="audits" className="text-xs">Audit Logs</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="transfers">
            <Card className="border-border/60 bg-card/65 backdrop-blur-md">
              <CardHeader>
                ...
                <CardDescription>Comprehensive records of all transfer operations, networks, and statuses.</CardDescription>
              </CardHeader>
              <CardContent>
                {transfersLoading ? (
                  ...
                  <p className="text-xs text-muted-foreground text-center py-8">No transfer events recorded.</p>
                ) : (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-xs">Debit Source</TableHead>
                          <TableHead className="text-xs">Recipient Details</TableHead>
                      ...
                      <TableBody>
                        {transfers.map((t) => (
                          <TableRow key={t.id}>
                            ...
                            <TableCell>
                              <div>
                                <p className="font-semibold text-xs text-foreground">{t.customer?.fullName}</p>
                                <p className="text-[10px] text-muted-foreground">{t.customer?.email}</p>
                              </div>
                            </TableCell>
                            <TableCell className="text-xs uppercase tracking-wider font-semibold text-[10px]">
                              {t.type.replace("_", " ")}
                            </TableCell>
                            <TableCell className="font-mono text-xs">{t.sourceAccount?.accountNumber || "N/A"}</TableCell>
          ...
          <TabsContent value="audits">
            <Card className="border-border/60 bg-card/65 backdrop-blur-md">
              <CardHeader>
                ...
                <CardDescription>Tamper-proof event logs detailing maker-checker review activities and transaction approvals.</CardDescription>
              </CardHeader>
              <CardContent>
                {auditLoading ? (
                  <div className="text-center py-12 text-sm text-muted-foreground animate-pulse">
                  ...
                  <p className="text-xs text-muted-foreground text-center py-8">No auditable system actions recorded.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-xs">Entity Target</TableHead>
                          <TableHead className="text-xs">Actor Employee</TableHead>
                      ...
                      <TableBody>
                        {auditLogs.map((log) => (
                          <TableRow key={log.id}>
                            <TableCell className="font-mono text-[10px] text-muted-foreground">{log.id.slice(0, 8).toUpperCase()}...</TableCell>
                            <TableCell className="font-mono text-xs font-semibold text-foreground">{log.action}</TableCell>
                            ...
                            <TableCell className="text-xs">
                              {log.actor ? (
                                <div>
                                  <p className="font-semibold text-xs text-foreground">{log.actor.fullName}</p>
                                  <p className="text-[10px] text-muted-foreground">{log.actor.email}</p>
                                </div>
                              ) : (
                                <span className="text-muted-foreground italic">System / Customer</span>
                              )}
                            </TableCell>
                            <TableCell>
                              {log.actor ? (
                                <Badge variant="secondary" className="text-[9px] uppercase px-1">{log.actor.role}</Badge>
                              ) : (
                                <span>-</span>
                              )}
                            </TableCell>
